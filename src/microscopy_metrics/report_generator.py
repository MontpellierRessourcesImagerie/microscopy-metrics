...
class Report_Generator(object):
    ...
    def draw_single_bead_report_pdf(self,active_path,index):
        ...
        data = [
            ["","Z","Y","X"],
            ["FWHM (µm)", f"{self._analysis_data[index]["FWHM"][0]:.4f}",f"{self._analysis_data[index]["FWHM"][1]:.4f}",f"{self._analysis_data[index]["FWHM"][2]:.4f}"],
            ["Uncertainty",f"{self._analysis_data[index]["uncertainty"][0][3]:.4f}",f"{self._analysis_data[index]["uncertainty"][1][3]:.4f}",f"{self._analysis_data[index]["uncertainty"][2][3]:.4f}"],
            ["Determination",f"{self._analysis_data[index]["determination"][0]:.4f}",f"{self._analysis_data[index]["determination"][1]:.4f}",f"{self._analysis_data[index]["determination"][2]:.4f}",]
        ]
    ...
    def generate_pdf_report(self,image_path):
        ...
        if self._parameters_detection["selected_tool"] == 0:
            textLines.append(f"Minimal distance: {self._parameters_detection['Min_dist']}")
        else:
        ...
        if self._parameters_detection["auto_threshold"]:
            textLines.append(f"Threshold tool: {self._parameters_detection['threshold_choice']}")
        else:
            textLines.append(f"Threshold relative: {self._parameters_detection['Rel_threshold']}")
        textLines.extend([
    ...
    def generate_csv_report(self,output_path):
        ...
        with open(output_path, mode='w',newline='') as file :
            ...
            for i,psf in enumerate(self._analysis_data):
                ...
                data_bead = [
                    ['','Z','Y','X'],
                    ['Centroid coordinates',f'{self._filtered_beads[i][0]}',f'{self._filtered_beads[i][1]}',f'{self._filtered_beads[i][2]}']
                ]
                data_fitting = [
                    ["","Z","Y","X"],
                    ["FWHM", f"{self._analysis_data[i]["FWHM"][0]}",f"{self._analysis_data[i]["FWHM"][1]}",f"{self._analysis_data[i]["FWHM"][2]}"],
                    ["Uncertainty",f"{self._analysis_data[i]["uncertainty"][0][3]}",f"{self._analysis_data[i]["uncertainty"][1][3]}",f"{self._analysis_data[i]["uncertainty"][2][3]}"],
                    ["Determination",f"{self._analysis_data[i]["determination"][0]}",f"{self._analysis_data[i]["determination"][1]}",f"{self._analysis_data[i]["determination"][2]}",]
                ]
                data_metrics = [
                    ["Metric","Value"],
