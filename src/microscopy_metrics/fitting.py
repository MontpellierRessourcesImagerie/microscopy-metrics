...
class Fitting(object):
    ...
    def gauss_2d(self,amp,bg,mu_x,mu_y,cxx,cxy,cyy):
        ...
        def fun(coords):
            ...
            exponent = -0.5 * (cov_inv[0,0] * (coords[:, 0] - mu_x) ** 2
    ...
    def process_single_fit(self,index):
        ...
        physic = [int(self._centroids[index][0]), int(self._centroids[index][1] - self._rois[index][0][1]), int(self._centroids[index][2] - self._rois[index][0][2])]
        ...
        for u in range(3):
            ...
            bg = np.median(psf[u])
            amp = psf[u].max() - bg
            sigma = np.sqrt(self.get_cov_matrix(np.clip(psf[u] - bg, 0, psf[u].max()), [self._spacing[u]], (self._centroids[index] - self._rois[index][0])))
            mu = np.argmax(psf[u])
    ...
    def get_coords(self,psf,axe1,axe2):
        ...
        """
        yy = np.arange(psf.shape[0]) * self._spacing[axe1]
        xx = np.arange(psf.shape[1]) * self._spacing[axe2]
    ...
    def process_single_fit_2D(self,index):
        ...
        for u in range(3):
            ...
            if u+1 < 3 :
                sigma = [params_1D[u][3],0.1,params_1D[u+1][3]]
                u2 = u+1
            else :
                sigma = [params_1D[u][3],0.1,params_1D[0][3]]
                u2 = 0
            if u+1 < 3 :
            ...
            pcov[6,6] += pcovs_1D[u2][3,3]
            result[1][u] += (px_to_um(self.fwhm(params[4]),self._spacing[u]))
            result[1][u2] += (px_to_um(self.fwhm(params[6]),self._spacing[u2]))
            result[2].append(self.uncertainty(pcov))
    ...
    def compute_fitting_1D(self):
        ...
        with ThreadPoolExecutor() as executor : 
            futures = {executor.submit(self.process_single_fit,i) : i for i, roi in enumerate(self._rois)}
