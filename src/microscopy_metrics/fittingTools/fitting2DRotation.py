...
class Fitting2DRotation(FittingTool):
    ...
    def plotFit3d(self, center, param1D, outputPath, coords):
        psf = self.setNormalizedImage()
        axes = ["Z","Y","X"]
        params2D = [
            [self.parameters[0],self.parameters[1],self.parameters[2],self.parameters[3],self.parameters[5],self.parameters[6],self.thetas[0]],
            [self.parameters[0],self.parameters[1],self.parameters[3],self.parameters[4],self.parameters[6],self.parameters[7],self.thetas[1]],
            [self.parameters[0],self.parameters[1],self.parameters[2],self.parameters[4],self.parameters[5],self.parameters[7],self.thetas[2]]
        ]
        psfs = [
        ...
        for i in range (3):
            params = [param1D[0], param1D[1], param1D[2+i], param1D[5+i]]
    ...
    def processSingleFit(self, index):
        ...
        coordsTmp = [x, y, z]
        if self._show : 
            self.plotFit3d(physic,params1D,activePath,coordsTmp)
            for i in range(3):
