...
class Fitting2DEllips(FittingTool):
    ...
    def fitCurve(self, amp, bg, mu, sigma, coords, psf):
        ...
        popt, pcov = curve_fit(
            ...
            p0=params,
            maxfev=5000,
            bounds=([0, -np.inf, 0, 0, 1e-6, 1e-6,1e-6],[np.inf, np.inf, psf.shape[0], psf.shape[1], np.inf,np.inf,np.inf])
        )
        return popt, pcov
    ...
    def show2dFit(self, psf, outputPath,params):
        ...
        fit = Fitting2D().gauss(*params)(fine_coords_yx)
    ...
    def plotFit3d(self, center, params, popt, outputPath, coords):
        ...
        for i in range (3):
            ...
            self.plotSingleFit(coords[i],psfs[i],fine,Fitting1D().gauss(*params[i])(fine),Fitting2D().gauss(*params2D[i])(fineCoords),axes[i],outputPath,[popt[0],popt[1],popt[2+i]])
    ...
    def processSingleFit(self, index):
        ...
        for u in range(3):
            ...
            params = [params[0],params[1],params[2], params[3], s1,s2]
